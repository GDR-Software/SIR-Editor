...
#if 0
#include <llvm-15/llvm/Demangle/ItaniumDemangle.h>
...
0};
  ...
  void* allocateMassive(size_t NBytes) {
    ...
    BlockMeta* NewMeta = reinterpret_cast<BlockMeta*>(std::malloc(
NBytes));
    if (NewMeta == nullptr)
      std::terminate();
    BlockList->Next = new (NewMeta) BlockMeta{BlockList->Next, 
0};
...
__cxa_demangle(const char *MangledName, char *Buf, size_t *N, int *Status) {
  ...
  Demangler Parser(MangledName, MangledName + std::strlen(
MangledName));
